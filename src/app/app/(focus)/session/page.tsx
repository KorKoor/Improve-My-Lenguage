...
export default async function SessionPage({ searchParams }: { searchParams: Promise<{ minutes?: string; focus?: string; surprise?: string; lesson?: string; again?: string; phase?: string }> }) {
  ...
  return (
    <SessionRunner
      ...
      key={`${sp.lesson ?? ""}|${sp.phase ?? ""}|${sp.focus ?? ""}|${sp.minutes ?? ""}|${sp.surprise ?? ""}|${sp.again ?? ""}`}
      minutes={phase ? 5 : minutes}
      focus={phase ? `phase:${phase.id}` : lesson ? `lesson:${lesson}` : (sp.focus ?? null)}
      ...
      title={phase ? phase.title : lesson ? `Lección ${lesson}` : sp.focus === "leeches" ? "Palabras rebeldes" : sp.focus === "letters" ? "Repaso de letras" : "Sesión de estudio"}
    />
  );
}
