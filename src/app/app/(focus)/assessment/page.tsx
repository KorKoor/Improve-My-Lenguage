...
export default async function AssessmentPage({ searchParams }: { searchParams: Promise<{ restart?: string }> }) {
  ...
  return <AssessmentRunner languageName={learner.language.name} language={learner.language.code} rtl={learner.language.rtl} restart={sp.restart === "1"} alreadyAssessed={Boolean(learner.ul.assessedAt)} />;
}
